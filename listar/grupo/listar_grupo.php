...
if ($_SESSION['logado'] == $chave) {
    ...
    $busca = mysql_query("SELECT * FROM grupo WHERE situacao = 1 ORDER BY grupo ASC") or die("Não foi possivel realizar a busca de grupos no banco de dados" . mysql_error());
        ...
        <div id="tamanho" align="center"><br><br>
            <div id="send_grupo">
                <table align="center" cellpadding="4px">
                    <tr>
                        <td colspan="2">
                            <div id="tit_send_grupo"><br>Lista de Grupo(s)</div>
                        </td>
                    </tr>
                    <tr id="topo_tabela">
                        <td>Nome do Grupo</td>
                    ...
                    while ($lista = mysql_fetch_assoc($busca)) {
                        if ($cor == 0) {
                            ...
                            ?> 
                            <td style="text-transform:uppercase;"><?php echo $lista['grupo'] ?></td>
                            <td  align="center">
                                <?php
                                $idadmin = $lista['id_admin'];
                                if ($_SESSION['idUser'] == $idadmin) {
                                    ?>
                                    <a href="../../editar/grupo/editar_grupo.php?editar=<?php echo $lista['id_grupo'] ?>" target="meio">
                                        <?php
                                    } else {
                                        ...
                                        ?>
                                        <img src="../../web_tools/img/edit.png" /></a>&nbsp;&nbsp;
                                    <?php
                                    if ($_SESSION['idUser'] == $idadmin) {
                                        ?>
                                        <a href="../../inativar/grupo/inativar_grupo_bd.php?inativar=<?php echo $lista['id_grupo'] ?>" onclick="javascript:return confirm('Atenção!  Deseja INATIVAR este grupo?')">
                                            <?php
                                        } else {
                                            $buscaUser = mysql_query("select * from admin where id_admin = '$idadmin'")
                                                    or die("Erro ao realizar a busca do usuário. " . mysql_error());
                                    ...
                                    ?>
                                    <script>
                                        alert("O banco de dados não possui Grupos cadastrados!");
                                        window.history.back();
                                    </script>
                                    <?php
                                }
